...
Combinatorics is largely the art of counting things. This section shows you how to carry out some
basic counting arguments. The library has basic results on counting elements of finsets:
EXAMPLES: -/
-- QUOTE:
open Finset
...
example (h : Function.Injective f) : #(s.image f) = #s := by
  rw [card_image_of_injective _ h]

example (h : Set.InjOn f s) : #(s.image f) = #s := by
  rw [card_image_of_injOn h]
-- QUOTE.
...
example (n : ℕ) : card (Fin n → α) = (card α)^n := by
  simp

variable {n : ℕ} {γ : Fin n → Type*} [∀ i, Fintype (γ i)]

example : card ((i : Fin n) → γ i) = ∏ i, card (γ i) := by
  simp

example : card (Σ i, γ i) = ∑ i, card (γ i) := by
  simp
-- QUOTE.
...
:math:`(n, n)`, not
including the diagonal. The cardinality of the full square is :math:`(n + 1)^2`; removing the size of
...
below describes the triangle as the union of the rows, where the :math:`j`th row consists of
...
def triangle (n : ℕ) : Finset (ℕ × ℕ) :=
  (range (n+1) ×ˢ range (n+1)).filter (fun p => p.1 < p.2)

-- EXAMPLES:
example (n : ℕ) : #(triangle n) = (n + 1) * n / 2 := by
  have : triangle n =
      (range (n+1)).biUnion (fun j ↦ (range j).image (., j)) := by
    ext p
    simp only [triangle, mem_filter, mem_product, mem_range,
        mem_biUnion, mem_image]
    constructor
    . rintro ⟨⟨hp1, hp2⟩, hp3⟩
      use p.2, hp2, p.1, hp3
...
/- TEXT:
The following variation on the proof does the calculation with fintypes instead of fintypes.
...
exactly fill the rectangle ``range n ×ˢ range (n + 1)``, which is what the calculation below does.
As an exercise, see if you can fill in the steps. In the solutions, we rely on ``omega``
...
EXAMPLES: -/
-- QUOTE:
def triangle' (n : ℕ) : Finset (ℕ × ℕ) := (range n ×ˢ range n).filter (fun p => p.1 ≤ p.2)
...
/- TEXT:
Let us close this section with an example and an exercise from a `tutorial <https://www.youtube.com/watch?v=_cJctOIXWE4&list=PLlF-CfQhukNn7xEbfL38eLgkveyk9_myQ&index=8&t=2737s&ab_channel=leanprovercommunity>`
...
consecutive integers, and hence to that are coprime.
...
EXAMPLES: -/
-- QUOTE:
example (m k : ℕ) (h : m ≠ k) (h' : m / 2 = k / 2) :
    m = k + 1 ∨ k = m + 1 := by omega
-- QUOTE.

/- TEXT:
The proof of the theorem uses the pigeonhole principle, in the form
