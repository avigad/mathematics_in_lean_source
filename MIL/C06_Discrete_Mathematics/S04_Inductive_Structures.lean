...
are generated inductively, from the bottom up.
...
For example, the data type ``List α`` is defined inductively:
EXAMPLES: -/
-- QUOTE:
...
theorem map_map (f : α → β) (g : β → γ) :
  ...
  | [] => rfl
  | a :: as => by
      rw [map_cons, map_cons, map_cons, map_map f g as]; rfl
-- QUOTE.
...
EXAMPLES: -/
-- QUOTE
theorem depth_le_sizeαα : ∀ t : BinTree, depth t ≤ size t := by
  sorry
-- QUOTE.
...
Prove the following:
EXAMPLES: -/
theorem size_flipαα : ∀ t, size (flipαα t) = size t := sorry
...
These are commonly written :math:`p_n`, :math:`\bot`, :math:`A \wedge B`,
...
Having defined the type of propositional formulas, we define what it means to evaluate a
...
/- TEXT:
The next definition defines the set of variables occurring in a formula, and the subsequent
...
theorem subst_eq_of_not_mem_varsαα :
    ∀ (A : PropForm) (n : ℕ) (C : PropForm), n ∉ A.vars →
      A.subst n C = A := sorry
-- QUOTE.
...
theorem subst_eq_of_not_mem_vars :
    ∀ (A : PropForm) (n : ℕ) (C : PropForm), n ∉ A.vars →
      A.subst n C = A
  | var m, n, C, h => by
      simp_all [subst, vars]; tauto
  | fls, n, C, _ => by rw [subst]
  | conj A B, n, C, h => by
      simp_all [subst, vars, subst_eq_of_not_mem_vars A,
        subst_eq_of_not_mem_vars B]
  | disj A B, n, C, h => by
      simp_all [subst, vars, subst_eq_of_not_mem_vars A,
        subst_eq_of_not_mem_vars B]
  | impl A B, n, C, h => by
