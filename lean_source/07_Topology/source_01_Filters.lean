...
* ``F.sets_of_superset : ∀ {U V}, U ∈ F.sets → U ⊆ V → V ∈ F.sets`` (if ``U`` belong to ``F.sets`` then anything larger also belongs)
...
to converges with respect to some ``G : filter Y`` along some ``F : filter X`` as
TEXT. -/
-- QUOTE:
...
``filter.map`` can be written as ``map``. This allows to rewrite the definition of ``tendsto`` using
...
/- TEXT:
Together these two properties allow to prove that limits compose, covering in one shot all 2197 variants from the introduction.
You can practice proving this statement using either the definition involving the quantifier or the algebraic definition
...
There also a pull-back operation going in the other direction: ``filter.comap`` and generalizing the
preimage operation on sets. For any given map ``f``,
``filter.map f`` and ``filter.comap f`` form a Galois connexion: 
...
``(x₀, y₀)`` are related to ``𝓝 x₀`` and ``𝓝 y₀``. There is product operation 
...
``F ⊓ G`` is the greatest filter that is smaller than bother ``F`` and ``G``, generalizing the notion
of the intersection of sets.

A lot of proofs in  mathlib use all this structure (``map``, ``comap``, ``inf``, ``sup``, ``prod``) 
...
(if ``U`` belong to ``F`` then anything larger also belongs), the first property 
...
to reformulate a ``tendsto f F G`` given bases for ``F`` and ``G``.
TEXT. -/
...
or sufficiently close to a given point. If  you did the exercices in :numref:`sequences_and_convergence`,
...
Then the fact ``at_top`` is a filter means in particular that the intersection of two elements of ``at_top`` 
...
specialized ``eventually_eq`` for the equality predicate. Note that both statement are
...
with ``eventually.and``. There is also a tactic ``filter_upwards`` that allows to nicely combines these. 
Compare:
...
There is dual version of ``∀ᶠ x in F, P x`` which is much less useful, but still exists: ``∃ᶠ x in F, P x`` means
